...
@Directive({
  selector: '[numbersOnly]' // TODO rename to [ngDecimalOnly]
})
export class NumbersOnlyDirective { // enables only 0-9 and one . characters

  constructor(private el: ElementRef) { }
  regexStr: string = '^[.0-9]+$';
  @HostListener('keydown', [ '$event' ])
  onKeyDown($event: KeyboardEvent) {
    if (this.el.nativeElement.value.includes('.') && $event.key.toLowerCase() === '.') {
      return false;
    } 
    if (
